...
class InputEventSystem{
    ...
    private:
        std::vector<std::unique_ptr<IPressed>> pressed;
        std::vector<std::unique_ptr<IPressedDown>> pressedDown;
        std::vector<std::unique_ptr<IPressedUp>> pressedUp;

        
};

void InputEventSystem::inputPos(GLFWwindow* window, double xpos, double ypos){
